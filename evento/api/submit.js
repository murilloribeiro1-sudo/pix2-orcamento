  ...
  try {
    ...
    if (task === 'selfie') {
      ...
      const prompt = body.prompt_override && isAdmin(body, req)
        ...
        : selfiePrompt();
      const out = await falSubmit(MODELS.selfie, {
        prompt,
        image_urls,
        num_images: 1,
        output_format: 'png',
    ...
    if (task === 'location') {
      ...
      const out = await falSubmit(MODELS.location, {
        prompt,
    ...
    if (task === 'video') {
      ...
      const locationUrl = clean(body.location_url, 1000);
      if (!prompt) return json(res, 400, { error: 'Prompt vazio.' });
      if (!/^https:\/\//.test(locationUrl)) return json(res, 400, { error: 'Gere a imagem do local antes.' });
      const refs = characterRefs(req);
